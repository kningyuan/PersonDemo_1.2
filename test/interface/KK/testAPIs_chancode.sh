...
#echo
pull=$(curl -s -X GET \
  "http://localhost:4000/channels/person1/chaincodes/interface?peer=peer0.org2.example.com&fcn=check_authority&args=%5b%22${idno}%22%2c%22${index}%22%5d" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json"
)
echo $pull
...
#echo
pull=$(curl -s -X GET \
  "http://localhost:4000/channels/person1/chaincodes/authority?peer=peer0.org2.example.com&fcn=query_authority_list&args=%5b%22${idno}%22%2c%22${index}%22%5d" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json"
)
echo $pull
...
#echo
pull=$(curl -s -X GET \
  "http://localhost:4000/channels/person1/chaincodes/interface?peer=peer0.org2.example.com&fcn=data_query&args=%5b%22${idno}%22%2c%22${index}%22%5d" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json"
)
...
#echo
pull=$(curl -s -X GET \
  "http://localhost:4000/channels/person1/chaincodes/index?peer=peer0.org2.example.com&fcn=index_query&args=%5b%22${idno}%22%5d" \
  -H "authorization: Bearer $ADMIN_TOKEN" \
  -H "content-type: application/json"
)
echo $pull
