...
function authority_register()
{
#echo "POST invoke chaincode on peers of Org1"
#echo
#echo " {$func} {$idno} {$index} "
#s_index=`echo ${index} | openssl aes-128-cbc -k 123 -base64 | tr "\n" "\^"`
curl -s -X POST \
  http://localhost:4000/channels/person1/chaincodes/authority \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json" \
  -d "{
	\"peers\": [\"peer0.org1.example.com\",\"peer1.org1.example.com\"],
	\"fcn\":\"authority_register\",
	\"args\":[\"$idno\",\"$index\"]
}"
#echo
#echo
...
#echo
pull=$(curl -s -X GET \
  "http://localhost:4000/channels/person1/chaincodes/authority?peer=peer0.org1.example.com&fcn=check_authority&args=%5b%22${idno}%22%2c%22${index}%22%5d" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json"
)
